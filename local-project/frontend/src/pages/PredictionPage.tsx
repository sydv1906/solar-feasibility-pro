...
export function PredictionPage() {
  ...
  const [lon, setLon] = useState<number>(78);
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      const marker = L.marker([20, 78]).addTo(map);
      markerRef.current = marker;

      map.on('click', (e: any) => {
        const { lat: clickLat, lng: clickLon } = e.latlng;
        const newLat = Math.round(clickLat * 10000) / 10000;
        const newLon = Math.round(clickLon * 10000) / 10000;
        setLat(newLat);
        setLon(newLon);
        marker.setLatLng([clickLat, clickLon]);
        // Reverse geocode to fill city input
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${newLat}&lon=${newLon}&format=json`)
          .then((r) => r.json())
          .then((data) => {
  ...
  function handleSuggestionClick(suggestion: NominatimResult) {
  ...
  async function handlePredict() {
    ...
    try {
      ...
      const todayHours = weather.hourly!.time.map((t, i) => ({ t, i })).filter(({ t }) => t.startsWith(todayDate));
      ...
      const hourlyToday = todayHours.map(({ t, i }) => {
        ...
        dailyTotal += pw;
        return { hour: `${hour.getHours()}:00`, power: Math.round((pw / 1000) * 100) / 100 };
      });
      ...
      const weeklyDays = (weather.daily?.time ?? []).map((dateStr, i) => {
        ...
        const dayPower = predictPower(peakShortwave, zenith, weather.daily!.cloudMean[i], weather.daily!.tempMax[i], weather.humidity, capacityKw) * 8;
        return { day: DAYS[d.getDay()], power: Math.round((dayPower / 1000) * 10) / 10 };
      });
      ...
      const monthlyDays = Array.from({ length: 30 }, (_, i) => ({
        day: `D${i + 1}`,
        power: Math.round((Math.max(0, avgDaily * (0.8 + Math.random() * 0.4)) / 1000) * 10) / 10,
      }));
      const monthlyTotal = monthlyDays.reduce((s, d) => s + d.power * 1000, 0);

      const MONTH_NAMES = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
      const seasonalScale = [0.6,0.7,0.85,1.0,1.2,1.3,1.3,1.2,1.0,0.85,0.7,0.6];
  ...
  const feasibilityConfig = {
    excellent: { color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', badge: 'bg-emerald-500 text-white', icon: <CheckCircle className="w-4 h-4" />, label: 'Excellent' },
    good: { color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', badge: 'bg-blue-500 text-white', icon: <CheckCircle className="w-4 h-4" />, label: 'Good' },
    moderate: { color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/30', badge: 'bg-amber-500 text-white', icon: <AlertCircle className="w-4 h-4" />, label: 'Moderate' },
    poor: { color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/30', badge: 'bg-red-500 text-white', icon: <XCircle className="w-4 h-4" />, label: 'Poor' },
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-lg">
            Enter your location and system capacity to get ML-powered solar generation forecasts.
          </p>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-2">
                Click on the map to select a location, or type a city name above
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="lat-input" className="text-xs text-slate-400 mb-1 block">Latitude</label>
                  <input
                    id="lat-input"
                    type="number"
                    value={lat}
                    onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) { setLat(v); updateMarker(v, lon); } }}
                ...
                <div>
                  <label htmlFor="lon-input" className="text-xs text-slate-400 mb-1 block">Longitude</label>
                  <input
                    id="lon-input"
                    type="number"
                    value={lon}
                    onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) { setLon(v); updateMarker(lat, v); } }}
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-white focus:outline-none focus:border-solar-yellow text-sm"
              ...
              >
                {geoLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Navigation className="w-4 h-4 text-solar-yellow" />}
                Auto-detect my location
              </button>
              <div>
                <label htmlFor="capacity-input" className="text-xs text-slate-400 mb-1 block">Solar System Capacity (kW)</label>
                <input
                  id="capacity-input"
                  type="number"
                  value={capacityKw}
                  onChange={(e) => setCapacityKw(Math.max(0.1, parseFloat(e.target.value) || 5))}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-white focus:outline-none focus:border-solar-yellow text-sm"
                  min="0.1" max="1000" step="0.5"
                />
                <p className="text-xs text-slate-500 mt-1">Typical residential: 3–10 kW | Commercial: 10–100 kW</p>
              </div>
              {error && (
              ...
              >
                {loading ? (
                  <><Loader2 className="w-5 h-5 animate-spin" /> Fetching weather &amp; computing...</>
                ) : (
        ...
        {/* Loading state */}
        {loading && (
          <div className="solar-card rounded-2xl p-12 flex flex-col items-center justify-center mb-6">
            ...
            <p className="text-white font-semibold text-lg">Fetching real-time weather data...</p>
            <p className="text-slate-400 text-sm mt-1">Calculating solar angles &amp; running ML model</p>
          </div>
        )}
        ...
        {result && !loading && (
          <div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              {[
                { label: 'Daily Output', value: (result.daily / 1000).toFixed(2), unit: 'kWh', icon: <Sun className="w-5 h-5" />, color: 'text-solar-yellow' },
                { label: 'Weekly Output', value: (result.weekly / 1000).toFixed(1), unit: 'kWh', icon: <Zap className="w-5 h-5" />, color: 'text-blue-400' },
                { label: 'Monthly Output', value: (result.monthly / 1000).toFixed(0), unit: 'kWh', icon: <Wind className="w-5 h-5" />, color: 'text-purple-400' },
                { label: 'Yearly Output', value: (result.yearly / 1000).toFixed(0), unit: 'kWh', icon: <Zap className="w-5 h-5" />, color: 'text-emerald-400' },
              ].map((card) => (
                <div key={card.label} className="solar-card rounded-2xl p-5">
            ...
            <div className="solar-card rounded-2xl p-6 mb-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-white font-semibold text-lg">{result.locationName}</div>
                    <div className="text-slate-400 text-sm">{lat}°N, {lon}°E</div>
                  </div>
                </div>
                <span className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-bold ${feasibilityConfig[result.feasibility].badge}`}>
                  {feasibilityConfig[result.feasibility].icon}
                  {feasibilityConfig[result.feasibility].label} Solar Potential
                </span>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <WeatherCard icon={<Thermometer className="w-4 h-4 text-red-400" />} label="Temperature" value={`${result.weather.temperature}°C`} />
                <WeatherCard icon={<Droplets className="w-4 h-4 text-blue-400" />} label="Humidity" value={`${result.weather.humidity}%`} />
                <WeatherCard icon={<Cloud className="w-4 h-4 text-slate-400" />} label="Cloud Cover" value={`${result.weather.cloudCover}%`} />
                <WeatherCard icon={<Sun className="w-4 h-4 text-solar-yellow" />} label="Solar Irradiance" value={`${result.weather.shortwave} W/m²`} />
              </div>
            </div>

            {/* Analysis Tabs */}
            <div className="solar-card rounded-2xl p-6">
              <h2 className="text-white font-semibold text-lg mb-4">Generation Analysis</h2>
              <div className="flex gap-2 mb-6">
                {(['daily','weekly','monthly','yearly'] as AnalysisTab[]).map((tab) => (
                  <button
                    key={tab}
                    type="button"
                    onClick={() => setAnalysisTab(tab)}
                    className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${
                      analysisTab === tab ? 'bg-solar-yellow text-slate-900' : 'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'
                    }`}
                  >
              ...
              {analysisTab === 'daily' && (
                <div>
                  <p className="text-slate-400 text-sm mb-4">Hourly solar generation curve for today</p>
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={result.hourlyToday}>
                      <defs><linearGradient id="lineGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f59e0b" stopOpacity={0.4}/><stop offset="95%" stopColor="#f59e0b" stopOpacity={0}/></linearGradient></defs>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                      <XAxis dataKey="hour" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                      <YAxis tick={{ fill: '#94a3b8', fontSize: 11 }} unit=" kWh" />
                      <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8 }} labelStyle={{ color: '#f1f5f9' }} formatter={(v: number) => [`${v} kWh`, 'Generation']} />
                      <Line type="monotone" dataKey="power" stroke="#f59e0b" strokeWidth={2.5} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                  <InsightCards cloudCover={result.weather.cloudCover} daily={result.daily} weather={result.weather} />
                </div>
              )}
              {analysisTab === 'weekly' && (
                <div>
                  <p className="text-slate-400 text-sm mb-4">Next 7 days predicted generation</p>
                  <ResponsiveContainer width="100%" height={260}>
                  ...
                  <InsightCards cloudCover={result.weather.cloudCover} daily={result.daily} weather={result.weather} period="weekly" weeklyDays={result.weeklyDays} />
                </div>
              )}
              {analysisTab === 'monthly' && (
                <div>
                  <p className="text-slate-400 text-sm mb-4">30-day generation breakdown</p>
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={result.monthlyDays}>
                      ...
                      <YAxis tick={{ fill: '#94a3b8', fontSize: 11 }} unit=" kWh" />
                      <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8 }} labelStyle={{ color: '#f1f5f9' }} formatter={(v: number) => [`${v} kWh`, 'Generation']} />
                      <Bar dataKey="power" fill="#f97316" radius={[3, 3, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                  <InsightCards cloudCover={result.weather.cloudCover} daily={result.daily} weather={result.weather} />
                </div>
              ...
              {analysisTab === 'yearly' && (
                <div>
                  <p className="text-slate-400 text-sm mb-4">Estimated yearly generation trend</p>
                  <ResponsiveContainer width="100%" height={260}>
                    <AreaChart data={result.yearlyMonths}>
                      <defs><linearGradient id="yearGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.4}/><stop offset="95%" stopColor="#10b981" stopOpacity={0.05}/></linearGradient></defs>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                      <XAxis dataKey="month" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                      <YAxis tick={{ fill: '#94a3b8', fontSize: 11 }} unit=" kWh" />
                      <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8 }} labelStyle={{ color: '#f1f5f9' }} formatter={(v: number) => [`${v} kWh`, 'Generation']} />
                      <Area type="monotone" dataKey="power" stroke="#10b981" strokeWidth={2} fill="url(#yearGrad)" />
                    </AreaChart>
                  </ResponsiveContainer>
                  <InsightCards cloudCover={result.weather.cloudCover} daily={result.daily} weather={result.weather} yearly={result.yearly} />
                </div>
              )}
            </div>
          </div>
        ...
        {/* Empty state */}
        {!result && !loading && (
          <div className="solar-card rounded-2xl p-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-white font-semibold text-xl mb-2">Ready to Predict</h3>
            <p className="text-slate-400 max-w-md">
              Select a location on the map or search by city name, set your solar capacity, and click Predict.
            </p>
          </div>
        )}
      </div>
...
function InsightCards({ cloudCover, daily, weather, period, weeklyDays, yearly }: {
  cloudCover: number; daily: number; weather: WeatherData;
  period?: string; weeklyDays?: { day: string; power: number }[]; yearly?: number;
}) {
  const cloudImpact = Math.round(cloudCover * 0.7);
  const efficiency = Math.round(Math.max(10, Math.min(95, (daily / 1000 / (weather.shortwave > 0 ? weather.shortwave * 0.024 : 1)) * 100)));
  ...
  const insights = [
    { icon: <Sun className="w-4 h-4 text-solar-yellow" />, text: `Highest generation: ${highestGen} kWh on ${bestDay}` },
    { icon: <Cloud className="w-4 h-4 text-slate-400" />, text: cloudImpact > 0 ? `Cloud cover reduced efficiency by ${cloudImpact}%` : 'Clear skies — maximum solar efficiency' },
    { icon: <Zap className="w-4 h-4 text-blue-400" />, text: `Estimated system efficiency: ${efficiency}%` },
    ...(annualSavings ? [{ icon: <CheckCircle className="w-4 h-4 text-emerald-400" />, text: `Estimated annual savings: ~$${annualSavings.toLocaleString()} (at $0.12/kWh)` }] : []),
    { icon: <AlertCircle className="w-4 h-4 text-solar-orange" />, text: 'Best generation window: 10:00 AM – 2:00 PM' },
  ];
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-5">
      {insights.map((ins) => (
        <div key={ins.text} className="bg-slate-800/60 rounded-xl p-4 flex items-start gap-3">
